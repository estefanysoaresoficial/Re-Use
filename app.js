...
/* ---------------------------------------------------------------------- */
/* CONTATO COM A LOJA — via Gmail (sem chave, sem cadastro, sem backend)  */
/* ---------------------------------------------------------------------- */
/* O site é 100% estático (GitHub Pages não roda backend). Em vez de usar */
/* um serviço externo com chave, o visitante escreve a mensagem aqui no   */
/* próprio site e, ao enviar, abre a tela de composição do Gmail          */
/* (mail.google.com) numa nova aba, já preenchida com o destinatário      */
/* (e-mail da loja), assunto e o texto — sem precisar digitar nada de     */
/* novo. O próprio visitante só confirma o envio na aba do Gmail.         */
function abrirComposeGmail(paraEmail, assunto, corpo){
  var url = "https://mail.google.com/mail/?view=cm&fs=1"
    + "&to=" + encodeURIComponent(paraEmail)
...
var EMPRESAS = [
  {
    ...
    cor:"linear-gradient(135deg,#E8B25A,#C97A2E)", email:"contato@paododia.com.br"
  },
  {
    ...
    cor:"linear-gradient(135deg,#8CC152,#4F8A3E)", email:"contato@vidaverdehortifruti.com.br"
  },
  {
    ...
    cor:"linear-gradient(135deg,#E3672F,#A8431B)", email:"contato@sabordacasa.com.br"
  },
  {
    ...
    cor:"linear-gradient(135deg,#4A90A4,#2C5F70)", email:"contato@boacompra.com.br"
  },
  {
    ...
    cor:"linear-gradient(135deg,#9C6B3E,#5E3E22)", email:"contato@cafeepao.com.br"
  },
  {
    ...
    cor:"linear-gradient(135deg,#D97AA6,#9C4770)", email:"contato@doceencanto.com.br"
  },
  {
    ...
    cor:"linear-gradient(135deg,#B7C15A,#748034)", email:"contato@feiradaroca.com.br"
  },
  {
    ...
    cor:"linear-gradient(135deg,#B08D57,#6E5228)", email:"contato@graonobre.com.br"
  }
];
...
/* ---------------------------------------------------------------------- */
/* CONTATO / CHAT COM A LOJA                                              */
/* ---------------------------------------------------------------------- */

var modalContato = document.getElementById("modalContato");
var contatoAlvo = null; // empresa atualmente selecionada no formulário
var CONTATO_INTERVALO_MIN_MS = 20000; // limite simples contra envio excessivo

function abrirModalContato(emp){
  if(!emp || !emp.email) return;
  contatoAlvo = emp;
  document.getElementById("contatoLojaTag").textContent = emp.nome;
  document.getElementById("ctNome").value = estado.perfil.nome || "";
  document.getElementById("ctEmail").value = estado.perfil.email || "";
  document.getElementById("ctMensagem").value = "";
  document.getElementById("ctEmpresa2").value = "";
  var aviso = document.getElementById("avisoContato");
  aviso.textContent = ""; aviso.className = "aviso";
  var btn = document.getElementById("btnEnviarContato");
  btn.disabled = false; btn.removeAttribute("aria-busy"); btn.textContent = "Enviar mensagem";
  if(typeof modalContato.showModal === "function") modalContato.showModal();
}
function fecharModalContato(){ modalContato.close(); }
document.getElementById("btnFecharContato").addEventListener("click", fecharModalContato);
...
document.getElementById("formContato").addEventListener("submit", function(e){
  ...
  if(!contatoAlvo || !contatoAlvo.email){
    aviso.textContent = "Não foi possível identificar a loja de destino."; aviso.className = "aviso aviso--erro";
    return;
  }
  // honeypot: se o campo invisível foi preenchido, é bot — finge sucesso e não envia nada
  if(document.getElementById("ctEmpresa2").value.trim() !== ""){
    aviso.textContent = "✓ Mensagem enviada com sucesso!"; aviso.className = "aviso aviso--ok";
    modalContato.close();
    return;
  }
  ...
  if(!nome || !email || !mensagem){
    aviso.textContent = "Preencha nome, e-mail e mensagem."; aviso.className = "aviso aviso--erro";
    return;
  }
  if(!emailValido(email)){
    aviso.textContent = "Informe um e-mail válido."; aviso.className = "aviso aviso--erro";
    return;
  }
  if(mensagem.length < 5){
    aviso.textContent = "Escreva uma mensagem um pouco mais detalhada."; aviso.className = "aviso aviso--erro";
    return;
  }
  if(mensagem.length > 600){
    aviso.textContent = "Mensagem muito longa (máximo 600 caracteres)."; aviso.className = "aviso aviso--erro";
    return;
  }
  var ultimoEnvio = Number(localStorage.getItem("reuse-brasil-ultimo-contato") || 0);
  if(Date.now() - ultimoEnvio < CONTATO_INTERVALO_MIN_MS){
    aviso.textContent = "Aguarde um instante antes de enviar outra mensagem."; aviso.className = "aviso aviso--erro";
    return;
  }

  var assunto = "Contato via ReUse Brasil — " + nome;
  var corpo =
    "Nova mensagem recebida pelo ReUse Brasil\n\n" +
    "Loja: " + contatoAlvo.nome + "\n" +
    "Visitante: " + nome + "\n" +
    "E-mail do visitante: " + email + "\n" +
    "Data: " + new Date().toLocaleString("pt-BR") + "\n\n" +
    "Mensagem:\n" + mensagem;

  abrirComposeGmail(contatoAlvo.email, assunto, corpo);
  localStorage.setItem("reuse-brasil-ultimo-contato", String(Date.now()));
  aviso.textContent = "✓ Abrimos o Gmail numa nova aba, já com tudo preenchido — é só clicar em Enviar por lá."; aviso.className = "aviso aviso--ok";
  setTimeout(function(){ modalContato.close(); }, 2200);
});
...
function paginaEmpresa(id){
      ...
      '<div class="perfil-empresa">' +
        ...
        '<div class="perfil-empresa__info">' +
          ...
          '<div class="perfil-empresa__acoes">' +
            ...
            (emp.email ? '<button class="btn btn--peq" id="btnContatoLoja">💬 Falar com a loja</button>' : '') +
          '</div>' +
        '</div>' +
      '</div>' +
  ...
  return {html:html, init:function(){
    ...
    var bc = document.getElementById("btnContatoLoja");
    if(bc) bc.addEventListener("click", function(){ abrirModalContato(emp); });
  }};
}
...
window.addEventListener("hashchange", function(){
  ...
  if(modalContato && modalContato.open) modalContato.close();
  renderizar(); window.scrollTo({top:0,behavior:"smooth"});
});
